const getOptions = () => {
  return {
    arrayDataWrap: Array.from($("input[name=arrayDataWrap]")).find(
      (el) => el.checked
    ).value,
    arrayWrap: Array.from($("input[name=arrayWrap]")).find((el) => el.checked)
      .value,
    arrayJoin: Array.from($("input[name=arrayJoin]")).find((el) => el.checked)
      .value,
    caseSensitivity: Array.from($("input[name=caseSensitivity]")).find(
      (el) => el.checked
    ).value,
  };
};
...
$(window).ready(() => {
  ...
  ta1.addEventListener(
    "input",
    function (e) {
      numLines[0].innerHTML = `${
  ...
  ta2.addEventListener(
    "input",
    function (e) {
      numLines[1].innerHTML = `${
        ta2.value.split("\n").filter(Boolean).length
      } lines`;
    },
    false
  ...
  ta3.addEventListener(
    "input",
    function (e) {
      numLines[2].innerHTML = `${
        ta3.value.split("\n").filter(Boolean).length
      } lines`;
    },
    false
